...
brew install anomalyco/tap/lotioncode # macOS og Linux (anbefalt, alltid oppdatert)
brew install lotioncode              # macOS og Linux (offisiell brew-formel, oppdateres sjeldnere)
...
LotionCode er også tilgjengelig som en desktop-app. Last ned direkte fra [releases-siden](https://github.com/anomalyco/lotioncode/releases) eller [lotioncode.ai/download](https://lotioncode.ai/download).
...
1. `$LOTIONCODE_INSTALL_DIR` - Egendefinert installasjonsmappe
...
LotionCode har to innebygde agents du kan bytte mellom med `Tab`-tasten.
...
Les mer om [agents](https://lotioncode.ai/docs/agents).

### Dokumentasjon

For mer info om hvordan du konfigurerer LotionCode, [**se dokumentasjonen**](https://lotioncode.ai/docs).

### Bidra

Hvis du vil bidra til LotionCode, les [contributing docs](./CONTRIBUTING.md) før du sender en pull request.

### Bygge på LotionCode

Hvis du jobber med et prosjekt som er relatert til LotionCode og bruker "lotioncode" som en del av navnet; for eksempel "lotioncode-dashboard" eller "lotioncode-mobile", legg inn en merknad i README som presiserer at det ikke er bygget av LotionCode-teamet og ikke er tilknyttet oss på noen måte.
...
- 100% open source
- Ikke knyttet til en bestemt leverandør. Selv om vi anbefaler modellene vi tilbyr gjennom [LotionCode Zen](https://lotioncode.ai/zen); kan LotionCode brukes med Claude, OpenAI, Google eller til og med lokale modeller. Etter hvert som modellene utvikler seg vil gapene lukkes og prisene gå ned, så det er viktig å være provider-agnostic.
- LSP-støtte rett ut av boksen
- Fokus på TUI. LotionCode er bygget av neovim-brukere og skaperne av [terminal.shop](https://terminal.shop); vi kommer til å presse grensene for hva som er mulig i terminalen.
- Klient/server-arkitektur. Dette kan for eksempel la LotionCode kjøre på maskinen din, mens du styrer den eksternt fra en mobilapp. Det betyr at TUI-frontend'en bare er en av de mulige klientene.

---

**Bli med i fellesskapet** [Discord](https://discord.gg/lotioncode) | [X.com](https://x.com/lotioncode)
