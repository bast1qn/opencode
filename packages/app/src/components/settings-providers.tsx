...
import { Tag } from "@lotioncode-ai/ui/tag"
import { showToast } from "@lotioncode-ai/ui/toast"
import { iconNames, type IconName } from "@lotioncode-ai/ui/icons/provider"
...
export const SettingsProviders: Component = () => {
  ...
  const connected = createMemo(() => {
    return providers
      .connected()
      .filter((p) => p.id !== "lotioncode" || Object.values(p.models).find((m) => m.cost?.input))
  })
                ...
                <div class="flex flex-wrap items-center justify-between gap-4 min-h-16 py-3 border-b border-border-weak-base last:border-none">
                  <div class="flex flex-col min-w-0">
                    <div class="flex items-center gap-x-3">
                      ...
                      <Show when={item.id === "lotioncode"}>
                        <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
                      </Show>
                    </div>
                    <Show when={item.id === "lotioncode"}>
                      <span class="text-12-regular text-text-weak pl-8">
                        {language.t("dialog.provider.lotioncode.note")}
                      </span>
                    </Show>
