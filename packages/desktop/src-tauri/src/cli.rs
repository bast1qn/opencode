...
const CLI_INSTALL_DIR: &str = ".lotioncode/bin";
const CLI_BINARY_NAME: &str = "lotioncode";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("lotioncode-install.sh");
...
pub fn create_command(app: &tauri::AppHandle, args: &str) -> Command {
    ...
    return app
        .shell()
        .sidecar("lotioncode-cli")
        .unwrap()
        .args(args.split_whitespace())
        .env("LOTIONCODE_EXPERIMENTAL_ICON_DISCOVERY", "true")
        .env("LOTIONCODE_EXPERIMENTAL_FILEWATCHER", "true")
        .env("LOTIONCODE_CLIENT", "desktop")
