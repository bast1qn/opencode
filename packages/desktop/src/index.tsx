...
import { AppBaseProviders, AppInterface, PlatformProvider, Platform } from "@lotioncode-ai/app"
...
import { Splash } from "@lotioncode-ai/ui/logo"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__LOTIONCODE__ ??= {}
  const pending = window.__LOTIONCODE__.deepLinks ?? []
  window.__LOTIONCODE__.deepLinks = [...pending, ...urls]
...
const createPlatform = (password: Accessor<string | null>): Platform => ({
    ...
    await Promise.resolve()
      .then(() => {
        const notification = new Notification(title, {
          body: description ?? "",
          icon: "https://lotioncode.ai/favicon-96x96-v3.png",
        })
  ...
  fetch: (input, init) => {
    ...
    const addHeader = (headers: Headers, password: string) => {
      headers.append("Authorization", `Basic ${btoa(`lotioncode:${password}`)}`)
    }
      ...
      <AppBaseProviders>
        <ServerGate>
          {(data) => {
            ...
            window.__LOTIONCODE__ ??= {}
            window.__LOTIONCODE__.serverPassword = data().password ?? undefined
