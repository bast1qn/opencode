import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@lotioncode-ai/sdk/v2"
import { SessionTurn } from "@lotioncode-ai/ui/session-turn"
import { SessionReview } from "@lotioncode-ai/ui/session-review"
import { DataProvider } from "@lotioncode-ai/ui/context"
import { DiffComponentProvider } from "@lotioncode-ai/ui/context/diff"
import { CodeComponentProvider } from "@lotioncode-ai/ui/context/code"
import { WorkerPoolProvider } from "@lotioncode-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createEffect, createMemo, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@lotioncode-ai/ui/logo"
import { IconButton } from "@lotioncode-ai/ui/icon-button"
import { ProviderIcon } from "@lotioncode-ai/ui/provider-icon"
import { createDefaultOptions } from "@lotioncode-ai/ui/pierre"
import { iife } from "@lotioncode-ai/util/iife"
...
import { MessageNav } from "@lotioncode-ai/ui/message-nav"
import { preloadMultiFileDiff, PreloadMultiFileDiffResult } from "@pierre/diffs/ssr"
import { Diff as SSRDiff } from "@lotioncode-ai/ui/diff-ssr"
import { clientOnly } from "@solidjs/start"
import { type IconName } from "@lotioncode-ai/ui/icons/provider"
import { Meta, Title } from "@solidjs/meta"
import { Base64 } from "js-base64"

const ClientOnlyDiff = clientOnly(() => import("@lotioncode-ai/ui/diff").then((m) => ({ default: m.Diff })))
const ClientOnlyCode = clientOnly(() => import("@lotioncode-ai/ui/code").then((m) => ({ default: m.Code })))
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@lotioncode-ai/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/lotioncode-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | LotionCode</Title>
              </Show>
              <Meta name="description" content="lotioncode - The AI coding agent built for the terminal." />
                          ...
                          <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                            <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                              ...
                              <div class="flex gap-3 items-center">
                                <IconButton
                                  as={"a"}
                                  href="https://github.com/anomalyco/lotioncode"
                                  target="_blank"
                                  icon="github"
                                  variant="ghost"
                                />
                                <IconButton
                                  as={"a"}
                                  href="https://lotioncode.ai/discord"
                                  target="_blank"
                                  icon="discord"
                                  variant="ghost"
                                />
