...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for lotioncode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal lotioncode sessions
...
lotioncode acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via lotioncode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | lotioncode acp
```
...
### Mapping to LotionCode

ACP sessions map cleanly to lotioncode's internal session model:
