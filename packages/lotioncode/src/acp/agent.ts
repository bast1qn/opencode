...
import type { Event, OpencodeClient, SessionMessageResponse } from "@lotioncode-ai/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `lotioncode auth login` in the terminal",
        name: "Login with lotioncode",
        id: "lotioncode-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // LotionCode stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const lotioncodeProvider = providers.find((p) => p.id === "lotioncode")
    if (lotioncodeProvider) {
      if (lotioncodeProvider.models["big-pickle"]) {
        return { providerID: "lotioncode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(lotioncodeProvider.models))
      if (best) {
        return {
