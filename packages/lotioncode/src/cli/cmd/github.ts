...
const AGENT_USERNAME = "lotioncode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/lotioncode.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://lotioncode.ai/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/lotioncode-agent"
            const command =
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.lotioncode.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  lotioncode:
    if: |
      ...
      startsWith(github.event.comment.body, '/lotioncode')
    ...
    steps:
      ...
      - name: Run lotioncode
        uses: anomalyco/lotioncode/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.lotioncode.ai" : "https://lotioncode.ai"
      ...
      try {
        ...
        console.log("lotioncode session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.lotioncode.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/lotioncode,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to lotioncode...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("lotioncode-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `lotioncode/${type}-${hex}-${timestamp}`
        }
        return `lotioncode/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/lotioncode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[lotioncode session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the lotioncode infrastructure after your response",
