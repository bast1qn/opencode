    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: lotioncode mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in lotioncode.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .lotioncode/ subdirectory too)
  const candidates = [path.join(baseDir, "lotioncode.json"), path.join(baseDir, "lotioncode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".lotioncode", "lotioncode.json"), path.join(baseDir, ".lotioncode", "lotioncode.jsonc"))
  }
  ...
  // Default to lotioncode.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., lotioncode x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "lotioncode-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
