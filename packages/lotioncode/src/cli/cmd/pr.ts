...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run lotioncode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for lotioncode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found lotioncode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`lotioncode import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting lotioncode...")
        UI.println()

        // Launch lotioncode TUI with session ID if available
        const { spawn } = await import("child_process")
        const lotioncodeArgs = sessionId ? ["-s", sessionId] : []
        const lotioncodeProcess = spawn("lotioncode", lotioncodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          lotioncodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`lotioncode exited with code ${code}`))
          })
          lotioncodeProcess.on("error", reject)
        })
      },
    })
  },
