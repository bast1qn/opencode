...
import { EOL } from "os"
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@lotioncode-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run lotioncode with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running lotioncode server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.LOTIONCODE_AUTO_SHARE && !args.share) return
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://lotioncode.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
