...
export function DialogModel(props: { providerID?: string }) {
  ...
  const options = createMemo(() => {
          ...
          return [
            {
              ...
              category: "Favorites",
              disabled: provider.id === "lotioncode" && model.id.includes("-nano"),
              footer: model.cost?.input === 0 && provider.id === "lotioncode" ? "Free" : undefined,
              onSelect: () => {
                dialog.clear()
                local.model.set(
                  {
    ...
    const providerOptions = pipe(
      sync.data.provider,
      sortBy(
        (provider) => provider.id !== "lotioncode",
        ...
        pipe(
          ...
          map(([model, info]) => {
            ...
            return {
              ...
              disabled: provider.id === "lotioncode" && model.includes("-nano"),
              footer: info.cost?.input === 0 && provider.id === "lotioncode" ? "Free" : undefined,
              onSelect() {
                dialog.clear()
                local.model.set(
                  {
