...
import lotioncode from "./theme/lotioncode.json" with { type: "json" }
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const [store, setStore] = createStore({
      ...
      active: (sync.data.config.theme ?? kv.get("theme", "lotioncode")) as string,
      ready: false,
    })
    ...
    const values = createMemo(() => {
      return resolveTheme(store.themes[store.active] ?? store.themes.lotioncode, store.mode)
    })
