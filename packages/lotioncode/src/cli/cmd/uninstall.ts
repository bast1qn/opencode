...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall lotioncode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall LotionCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g lotioncode-ai",
      pnpm: "pnpm uninstall -g lotioncode-ai",
      bun: "bun remove -g lotioncode-ai",
      yarn: "yarn global remove lotioncode-ai",
      brew: "brew uninstall lotioncode",
      choco: "choco uninstall lotioncode",
      scoop: "scoop uninstall lotioncode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall lotioncode -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using LotionCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# lotioncode") || content.includes(".lotioncode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".lotioncode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".lotioncode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".lotioncode"))
    ) {
      continue
    }
