...
export namespace LSPServer {
    ...
    async spawn(root) {
      ...
      if (!binary) {
        ...
        if (!(await Bun.file(js).exists())) {
          if (Flag.LOTIONCODE_DISABLE_LSP_DOWNLOAD) return
  ...
  export const Ty: Info = {
    ...
    async spawn(root) {
      if (!Flag.LOTIONCODE_EXPERIMENTAL_LSP_TY) {
        return undefined
      }
  ...
  export const JDTLS: Info = {
    ...
    async spawn(root) {
      ...
      const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "lotioncode-jdtls-data"))
      return {
        process: spawn(
          java,
          [
