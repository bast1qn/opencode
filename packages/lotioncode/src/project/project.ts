...
import { fn } from "@lotioncode-ai/util/fn"
  ...
  export async function fromDirectory(directory: string) {
    ...
    const { id, sandbox, worktree, vcs } = await iife(async () => {
      ...
      if (git) {
        ...
        let id = await Bun.file(path.join(git, "lotioncode"))
          .text()
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.LOTIONCODE_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            void Bun.file(path.join(git, "lotioncode"))
              .write(id)
              .catch(() => undefined)
          }
        }
    ...
    if (Flag.LOTIONCODE_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
