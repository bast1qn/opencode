...
function globalFiles() {
  ...
  if (!Flag.LOTIONCODE_DISABLE_CLAUDE_CODE_PROMPT) {
    files.push(path.join(os.homedir(), ".claude", "CLAUDE.md"))
  }
  if (Flag.LOTIONCODE_CONFIG_DIR) {
    files.push(path.join(Flag.LOTIONCODE_CONFIG_DIR, "AGENTS.md"))
  }
  return files
}
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.LOTIONCODE_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no LOTIONCODE_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.LOTIONCODE_CONFIG_DIR, Flag.LOTIONCODE_CONFIG_DIR).catch(() => [])
}
