...
const managedConfigDir = process.env.LOTIONCODE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "lotioncode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "lotioncode.json") {
...
test("loads JSONC config file", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "lotioncode.jsonc"),
        `{
        // This is a comment
        "$schema": "https://lotioncode.ai/config.json",
        "model": "test/model",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "lotioncode.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "lotioncode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .lotioncode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const lotioncodeDir = path.join(dir, ".lotioncode")
      await fs.mkdir(lotioncodeDir, { recursive: true })
      const agentDir = path.join(lotioncodeDir, "agent")
...
test("loads agents from .lotioncode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const lotioncodeDir = path.join(dir, ".lotioncode")
      await fs.mkdir(lotioncodeDir, { recursive: true })

      const agentsDir = path.join(lotioncodeDir, "agents")
...
test("loads commands from .lotioncode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const lotioncodeDir = path.join(dir, ".lotioncode")
      await fs.mkdir(lotioncodeDir, { recursive: true })

      const commandDir = path.join(lotioncodeDir, "command")
...
test("loads commands from .lotioncode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const lotioncodeDir = path.join(dir, ".lotioncode")
      await fs.mkdir(lotioncodeDir, { recursive: true })

      const commandsDir = path.join(lotioncodeDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "lotioncode.json"),
        JSON.stringify({ $schema: "https://lotioncode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "lotioncode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .lotioncode config
      const projectDir = path.join(dir, "project")
      const lotioncodeDir = path.join(projectDir, ".lotioncode")
      await fs.mkdir(lotioncodeDir, { recursive: true })
      ...
      // Local .lotioncode config with different plugins
      await Bun.write(
        path.join(lotioncodeDir, "lotioncode.json"),
        JSON.stringify({
          $schema: "https://lotioncode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .lotioncode config with some overlapping plugins
      await Bun.write(
        path.join(lotioncodeDir, "lotioncode.json"),
        JSON.stringify({
          $schema: "https://lotioncode.ai/config.json",
...
// Note: preload.ts sets LOTIONCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .lotioncode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .lotioncode directory config enables it
      const lotioncodeDir = path.join(dir, ".lotioncode")
      await fs.mkdir(lotioncodeDir, { recursive: true })
      await Bun.write(
        path.join(lotioncodeDir, "lotioncode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/lotioncode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/lotioncode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-lotioncode@2.4.3")).toBe("oh-my-lotioncode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@lotioncode/plugin@2.0.0")).toBe("@lotioncode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-lotioncode@2.4.3", "file:///project/.lotioncode/plugin/oh-my-lotioncode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.lotioncode/plugin/oh-my-lotioncode.js")
  })
  ...
  test("local plugin directory overrides global lotioncode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const lotioncodeDir = path.join(projectDir, ".lotioncode")
        const pluginDir = path.join(lotioncodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("LOTIONCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["LOTIONCODE_DISABLE_PROJECT_CONFIG"]
    process.env["LOTIONCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["LOTIONCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["LOTIONCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .lotioncode/ directories when flag is set", async () => {
    const originalEnv = process.env["LOTIONCODE_DISABLE_PROJECT_CONFIG"]
    process.env["LOTIONCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .lotioncode directory with a command
          const lotioncodeDir = path.join(dir, ".lotioncode", "command")
          await fs.mkdir(lotioncodeDir, { recursive: true })
          await Bun.write(path.join(lotioncodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .lotioncode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["LOTIONCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["LOTIONCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["LOTIONCODE_CONFIG_DIR"]
      process.env["LOTIONCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["LOTIONCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["LOTIONCODE_CONFIG_DIR"]
      } else {
        process.env["LOTIONCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("LOTIONCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["LOTIONCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["LOTIONCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["LOTIONCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["LOTIONCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from LOTIONCODE_CONFIG_DIR, not project
