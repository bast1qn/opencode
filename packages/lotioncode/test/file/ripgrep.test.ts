  ...
  test("defaults to include hidden", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.mkdir(path.join(dir, ".lotioncode"), { recursive: true })
        await Bun.write(path.join(dir, ".lotioncode", "thing.json"), "{}")
      },
    })

    const files = await Array.fromAsync(Ripgrep.files({ cwd: tmp.path }))
    const hasVisible = files.includes("visible.txt")
    const hasHidden = files.includes(path.join(".lotioncode", "thing.json"))
