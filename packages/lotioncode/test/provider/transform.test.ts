...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const lotioncodeModel = {
      ...openaiModel,
      providerID: "lotioncode",
      api: {
        id: "lotioncode-test",
        url: "https://api.lotioncode.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, lotioncodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.lotioncode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.lotioncode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          lotioncode: { itemId: "msg_lotioncode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              lotioncode: { itemId: "msg_lotioncode_part" },
    ...
    expect(result[0].providerOptions?.lotioncode?.itemId).toBe("msg_lotioncode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.lotioncode?.itemId).toBe("msg_lotioncode_part")
