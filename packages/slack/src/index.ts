...
import { createOpencode, type ToolPart } from "@lotioncode-ai/sdk"
...
console.log("🚀 Starting lotioncode server...")
const lotioncode = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await lotioncode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new lotioncode session...")
    const { client, server } = lotioncode
    ...
    console.log("✅ Created lotioncode session:", createResult.data.id)
  ...
  console.log("📝 Sending to lotioncode:", message.text)
