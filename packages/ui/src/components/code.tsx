...
export function Code<T>(props: CodeProps<T>) {
  ...
  const clearHighlightFind = () => {
    ...
    if (!api) return
    api.delete("lotioncode-find")
    api.delete("lotioncode-find-current")
  }
  ...
  const setHighlights = (ranges: Range[], index: number) => {
    ...
    if (active) api.set("lotioncode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== index)
    if (rest.length > 0) api.set("lotioncode-find", new Highlight(...rest))
    return true
  }
