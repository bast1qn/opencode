...
description: What's new in LotionCode 1.0.
---

LotionCode 1.0 is a complete rewrite of the TUI.
...
The new TUI works like the old one since it connects to the same lotioncode server.
...
version. However some older versions of LotionCode always grab latest.

To upgrade manually, run

```bash
$ lotioncode upgrade 1.0.0
```

To downgrade back to 0.x, run

```bash
$ lotioncode upgrade 0.15.31
```
