...
There are two types of agents in LotionCode; primary agents and subagents.
...
LotionCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
LotionCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

LotionCode comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `lotioncode.json` config file:
...
- Global: `~/.config/lotioncode/agents/`
- Per-project: `.lotioncode/agents/`

```markdown title="~/.config/lotioncode/agents/review.md"
---
...
If no temperature is specified, LotionCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global LotionCode config and the project specific config.
...
The model ID in your LotionCode config uses the format `provider/model-id`. For example, if you're using [LotionCode Zen](/docs/zen), you would use `lotioncode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="lotioncode.json" {3-6,9-12}
{
  "$schema": "https://lotioncode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="lotioncode.json" {3-5,8-10}
{
  "$schema": "https://lotioncode.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="lotioncode.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Run `lotioncode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/lotioncode).
:::
...
```markdown title="~/.config/lotioncode/agents/docs-writer.md"
---
...
```markdown title="~/.config/lotioncode/agents/security-auditor.md"
---
