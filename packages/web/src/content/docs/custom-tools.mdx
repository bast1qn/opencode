---
title: Custom Tools
description: Create tools the LLM can call in lotioncode.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside lotioncode's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.lotioncode/tools/` directory of your project.
- Or globally, by placing them in `~/.config/lotioncode/tools/`.
...
```ts title=".lotioncode/tools/database.ts" {1}
...
```ts title=".lotioncode/tools/math.ts"
import { tool } from "@lotioncode-ai/plugin"
...
```ts title=".lotioncode/tools/project.ts" {8}
import { tool } from "@lotioncode-ai/plugin"
...
```python title=".lotioncode/tools/add.py"
import sys
...
```ts title=".lotioncode/tools/python-add.ts" {10}
import { tool } from "@lotioncode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".lotioncode/tools/add.py")
