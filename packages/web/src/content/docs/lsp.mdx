---
title: LSP Servers
description: LotionCode integrates with your LSP servers.
---

LotionCode integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

LotionCode comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `LOTIONCODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When lotioncode opens a file, it:
...
You can customize LSP servers through the `lsp` section in your lotioncode config.
...
```json title="lotioncode.json" {5-7}
{
  "$schema": "https://lotioncode.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="lotioncode.json" {5-9}
{
  "$schema": "https://lotioncode.ai/config.json",
  "lsp": {
    "typescript": {
