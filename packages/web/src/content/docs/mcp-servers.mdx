...
You can add external tools to LotionCode using the _Model Context Protocol_, or MCP. LotionCode supports both local and remote servers.
...
You can define MCP servers in your [LotionCode Config](https://lotioncode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="lotioncode.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/lotioncode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
```jsonc title="lotioncode.jsonc" {15}
{
  "$schema": "https://lotioncode.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
LotionCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, LotionCode will:
...
If the server requires authentication, LotionCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `lotioncode mcp auth <server-name>`.
...
```json title="lotioncode.json" {7-11}
{
  "$schema": "https://lotioncode.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
lotioncode mcp auth my-oauth-server
```
...
```bash
lotioncode mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, LotionCode will store the tokens securely in `~/.local/share/lotioncode/mcp-auth.json`.
...
lotioncode mcp debug my-oauth-server
```
...
Your MCPs are available as tools in LotionCode, alongside built-in tools. So you can manage them through the LotionCode config like any other tool.
...
```json title="lotioncode.json" {11, 14-18}
{
  "$schema": "https://lotioncode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="lotioncode.json" {4-8}
{
  "$schema": "https://lotioncode.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
lotioncode mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect LotionCode to your Sentry account.
...
```json title="lotioncode.json" {4-7}
{
  "$schema": "https://lotioncode.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="lotioncode.json" {7-9}
{
  "$schema": "https://lotioncode.ai/config.json",
  "mcp": {
    "context7": {
