...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start LotionCode.
...
Here are several models that work well with LotionCode, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [LotionCode Zen](/docs/zen), you would use `lotioncode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```jsonc title="lotioncode.jsonc" {7-12,19-24}
{
  "$schema": "https://lotioncode.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="lotioncode.jsonc" {6-21}
{
  "$schema": "https://lotioncode.ai/config.json",
  "provider": {
    "lotioncode": {
...
Many models support multiple variants with different configurations. LotionCode ships with built-in default variants for popular providers.

### Built-in variants

LotionCode ships with default variants for many providers:
...
```jsonc title="lotioncode.jsonc" {7-18}
{
  "$schema": "https://lotioncode.ai/config.json",
  "provider": {
    "openai": {
...
When LotionCode starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the LotionCode config.
