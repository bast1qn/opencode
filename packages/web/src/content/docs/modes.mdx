...
:::caution
Modes are now configured through the `agent` option in the lotioncode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in lotioncode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the lotioncode config.
...
lotioncode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.lotioncode/plans/*.md` to detail the plan itself
...
Configure modes in your `lotioncode.json` config file:
...
- Global: `~/.config/lotioncode/modes/`
- Project: `.lotioncode/modes/`

```markdown title="~/.config/lotioncode/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global lotioncode config and the project specific config.
...
```json title="lotioncode.json" {4-14}
{
  "$schema": "https://lotioncode.ai/config.json",
  "mode": {
    "docs": {
...
Create mode files in `.lotioncode/modes/` for project-specific modes or `~/.config/lotioncode/modes/` for global modes:

```markdown title=".lotioncode/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/lotioncode/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
