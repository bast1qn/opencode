---
title: Providers
description: Using any LLM provider in LotionCode.
---

import config from "../../../config.mjs"
export const console = config.console

LotionCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your LotionCode config.
...
in `~/.local/share/lotioncode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your LotionCode
config.
...
LotionCode Zen is a list of models provided by the LotionCode team that have been
tested and verified to work well with LotionCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with LotionCode Zen.
:::

1. Run the `/connect` command in the TUI, select lotioncode, and head to [lotioncode.ai/auth](https://lotioncode.ai/auth).
...
It works like any other provider in LotionCode and is completely optional to use.
...
To use Amazon Bedrock with LotionCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running lotioncode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY lotioncode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile lotioncode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX lotioncode
   ```
   ...
   For project-specific or persistent configuration, use `lotioncode.json`:

   ```json title="lotioncode.json"
   {
     "$schema": "https://lotioncode.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Using your Claude Pro/Max subscription in LotionCode is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for lotioncode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX lotioncode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX lotioncode
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your lotioncode config.
...
to store token in lotioncode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
LotionCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock LotionCode
to only use your own GitLab-hosted instance, add the following to your
`lotioncode.json` file. It is also recommended to disable session sharing.
...
More documentation on [lotioncode-gitlab-auth](https://www.npmjs.com/package/@gitlab/lotioncode-gitlab-auth) homepage.

##### Configuration

Customize through `lotioncode.json`:
...
{
  "$schema": "https://lotioncode.ai/config.json",
  "plugin": ["@gitlab/lotioncode-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with lotioncode:
...
To use Google Vertex AI with LotionCode:
...
2. Set the required environment variables:
   ...
   Set them while running lotioncode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id lotioncode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through lotioncode, you can always configure it yourself.
...
```jsonc title="~/.config/lotioncode/lotioncode.jsonc"
{
  "$schema": "https://lotioncode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [lotioncode-helicone-session](https://github.com/H2Shami/lotioncode-helicone-session) plugin to automatically log each LotionCode conversation as a session in Helicone.

```bash
npm install -g lotioncode-helicone-session
```
...
```json title="lotioncode.json"
{
  "plugin": ["lotioncode-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each LotionCode conversation listed as a separate session.
...
You can configure lotioncode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="lotioncode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://lotioncode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure lotioncode to use local models through LM Studio.

```json title="lotioncode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://lotioncode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure lotioncode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for LotionCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/lotioncode) for details.
:::

```json title="lotioncode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://lotioncode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with LotionCode:
...
3. Copy the API key for use in LotionCode.
...
6. **Important**: Before using cloud models in LotionCode, you must pull the model information locally:
...
LotionCode Zen is a list of tested and verified models provided by the LotionCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>LotionCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **LotionCode Zen**.

   ```txt
   /connect
   ```

3. Enter your LotionCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your lotioncode config.
...
5. You can also customize them through your lotioncode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' lotioncode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group lotioncode
   ```
...
You can also customize models through your lotioncode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with lotioncode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in lotioncode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `lotioncode.json` file in your project directory:

   ```json title="lotioncode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://lotioncode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="lotioncode.json" {9,11,17-20}
{
  "$schema": "https://lotioncode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow LotionCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `lotioncode auth list` to see if the credentials
...
2. For custom providers, check the lotioncode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your lotioncode config.
