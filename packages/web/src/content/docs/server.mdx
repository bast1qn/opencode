---
title: Server
description: Interact with lotioncode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `lotioncode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an lotioncode client can use.
...
```bash
lotioncode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Custom domain name for mDNS service | `lotioncode.local` |
| `--cors`        | Additional browser origins to allow | `[]`             |

`--cors` can be passed multiple times:

```bash
lotioncode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `LOTIONCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `lotioncode`, or set `LOTIONCODE_SERVER_USERNAME` to override it. This applies to both `lotioncode serve` and `lotioncode web`.

```bash
LOTIONCODE_SERVER_PASSWORD=your-password lotioncode serve
```
...
When you run `lotioncode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the lotioncode server to interact with lotioncode programmatically.
:::

This architecture lets lotioncode support multiple clients and allows you to interact with lotioncode programmatically.

You can run `lotioncode serve` to start a standalone server. If you have the
lotioncode TUI running, `lotioncode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the LotionCode [IDE](/docs/ide) plugins.
...
The lotioncode server exposes the following APIs.
