...
With LotionCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, LotionCode uses our own `lotioncode` theme.
...
LotionCode comes with several built-in themes.
...
- Want LotionCode to match their terminal's appearance
...
LotionCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/lotioncode/themes/*.json` or `$XDG_CONFIG_HOME/lotioncode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.lotioncode/themes/*.json`
4. **Current working directory** - Defined in `./.lotioncode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/lotioncode/themes
vim ~/.config/lotioncode/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .lotioncode/themes
vim .lotioncode/themes/my-theme.json
```
...
{
  "$schema": "https://lotioncode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
