...
Tools allow the LLM to perform actions in your codebase. LotionCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in LotionCode.
...
:::note
This tool is only available when `LOTIONCODE_EXPERIMENTAL_LSP_TOOL=true` (or `LOTIONCODE_EXPERIMENTAL=true`).
:::
